...
sid32 mutex, producedInitialValueMutex;
...
process producer(void)
{
	...
	int32 i, initialValueNotProduced = 1;
	...
	while(1){
		for(i = 0; i < 20; i++){
			...
			produce(i);
			mutex_release(mutex);
			if(initialValueNotProduced){
				initialValueNotProduced = 0;
				mutex_release(producedInitialValueMutex);
			}
		}	
	}
...
process consumer(void)
{
	...
	while(1){
		for(i = 0; i < 20; i++){
			...
			mutex_acquire(producedInitialValueMutex);
			mutex_acquire(mutex);
			consume();
			consumed_count += 1;
			mutex_release(mutex);
...
{
	mutex = semcreate(1);
	producedInitialValueMutex = semcreate(0);
	recvclr();
